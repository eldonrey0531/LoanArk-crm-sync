...
import { useTop100ByUpdated, useHubSpotSync } from "@/hooks/useContacts";
import { formatDistanceToNow } from "date-fns";
import { toast } from "@/hooks/use-toast";

export default function LatestUpdated() {
  const [searchTerm, setSearchTerm] = useState("");
  const { data: contacts, isLoading, error, refetch } = useTop100ByUpdated();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold">Latest Updated Contacts</h1>
          <p className="text-muted-foreground mt-1">Top 100 most recently updated contacts</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Contacts sorted by last update date, showing the 100 most recently modified entries.
          </p>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                    ...
                    <TableRow key={contact.id}>
                      ...
                      <TableCell className="text-sm">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                          new Date().getTime() - new Date(contact.updated_at).getTime() < 24 * 60 * 60 * 1000
