    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Real-time synchronization monitoring and control</p>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className={`w-3 h-3 rounded-full ${
                  ...
                  syncEnabled ? 'bg-success' : 'bg-muted-foreground'
                }`} />
                <span className="font-medium">
                  {isSyncing ? 'Syncing...' : syncEnabled ? 'Ready' : 'Paused'}
                </span>
              </div>
              <Badge variant={syncEnabled ? "secondary" : "outline"} className={
                syncEnabled ? "bg-success-muted text-success" : ""
              }>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Rate Limit: 85/100</p>
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">Last 100 synchronization operations</p>
        </CardHeader>
        <CardContent>
              ...
              <TableBody>
                {syncHistory.map((sync) => (
                  <TableRow key={sync.id}>
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        ...
                        <Badge 
                          variant="outline"
                          className={
                            sync.status === 'success' ? 'border-success text-success' :
                            sync.status === 'partial' ? 'border-warning text-warning' :
                    ...
                    <TableCell className="text-success">{sync.recordsUpdated}</TableCell>
                    <TableCell className="text-primary">{sync.recordsCreated}</TableCell>
                    <TableCell className="text-destructive">{sync.recordsFailed}</TableCell>
                    <TableCell className="font-mono text-sm">{sync.duration}</TableCell>
