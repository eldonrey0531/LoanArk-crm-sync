...
const auditLogs = [
  ...
  },
  {
    ...
    oldValues: {},
    newValues: { firstname: "Sarah", lastname: "Johnson", email: "sarah@example.com" },
    status: "success"
  },
  {
    id: 3,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Logs & Audit Trail</h1>
          <p className="text-muted-foreground mt-1">Complete activity and error tracking</p>
        </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="font-medium text-foreground">{log.contact}</span>
                          <span className="text-sm text-muted-foreground">#{log.contactId}</span>
                        </div>
                      ...
                      {log.changedFields.length > 0 && (
                        <div className="mt-2">
                          <span className="text-sm font-medium text-foreground">Changed fields: </span>
                          <span className="text-sm text-muted-foreground">
                      ...
                      {log.action === "update" && (
                        <div className="mt-2 text-sm">
                          {Object.entries(log.newValues).map(([field, newValue]) => (
                            <div key={field} className="text-muted-foreground">
                              <span className="font-mono">{field}:</span>{" "}
                              <span className="text-destructive line-through">{log.oldValues[field]}</span>{" "}
                              →{" "}
        ...
        <TabsContent value="errors">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {errorLogs.map((error) => (
                  <div key={error.id} className="p-4 border border-destructive/20 bg-destructive/5 rounded-lg">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          <XCircle className="w-4 h-4 text-destructive" />
                          <Badge variant="destructive">{error.type}</Badge>
                          <span className="font-medium text-foreground">{error.contact}</span>
                          <span className="text-sm text-muted-foreground">#{error.contactId}</span>
                        </div>
