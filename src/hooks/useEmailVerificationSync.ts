...
export function useEmailVerificationSync(): UseEmailVerificationSyncReturn {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      import('../services/supabaseEmailVerificationService').then(({ supabaseEmailVerificationService }) => {
        import('../services/hubspotEmailVerificationService').then(({ hubspotEmailVerificationService }) => {
          import('../services/emailVerificationSyncService').then(({ initializeEmailVerificationSyncService }) => {
   ...
   */
  const loadRecords = useCallback(async (params?: GetEmailVerificationRecordsParams) => {
    if (!isAuthenticated) {
   ...
   */
  const syncRecord = useCallback(async (record: SupabaseContact) => {
    if (!isAuthenticated) {
      throw new Error('User not authenticated');
    }
    ...
    try {
      ...
      setSyncErrors(prev => ({
        ...prev,
        [record.id]: error instanceof Error ? error.message : 'Unknown sync error'
      }));
   ...
   */
  const retrySync = useCallback(async (recordId: number) => {
  ...
  const combinedError = recordsHook.error?.message ||
